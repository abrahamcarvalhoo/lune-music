...
const Controls = ({ isPlaying, repeat, setRepeat, shuffle, setShuffle, currentSongs, handlePlayPause, handlePrevSong, handleNextSong }) => (
  <div className="flex items-center justify-around md:w-36 lg:w-52 2xl:w-80">
    <BsArrowRepeat size={20} color={repeat ? '#D9F99D' : 'white'} onClick={() => setRepeat((prev) => !prev)} className="hidden cursor-pointer sm:block" />
    ...
    <BsShuffle size={20} color={shuffle ? '#D9F99D' : 'white'} onClick={() => setShuffle((prev) => !prev)} className="hidden cursor-pointer sm:block" />
  </div>
);
